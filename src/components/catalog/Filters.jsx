...
import styles from "./catalog.module.css"; // Подключение CSS модулей
...
export const Filters = ({ onFilterChange }) => {
  ...
  const handleCheckboxChange = (type) => {
    ...
    onFilterChange({ generatorType: type, selectedPower });
  };
  ...
  const handlePowerChange = (power) => {
    setSelectedPower((prevSelectedPower) => {
      const newSelectedPower = prevSelectedPower.includes(power)
        ? prevSelectedPower.filter((p) => p !== power)
        : [...prevSelectedPower, power];
      onFilterChange({ generatorType, selectedPower: newSelectedPower });
      return newSelectedPower;
    });
  };
